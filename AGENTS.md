...
本仓库由 1 名软件负责人和 2 名固定组员共同维护，日常开发默认由 Codex 辅助完成。
...
3. 对应 GitHub Issue：负责人、截止时间、依赖和协作状态。
...
- 从最新 `main` 创建短期分支：`feature/<issue>-<slug>`、`fix/<issue>-<slug>`、`docs/<issue>-<slug>` 或 `chore/<issue>-<slug>`。
...
```bash
rtk python -m compileall -q smart_grid_core
rtk python -m smart_grid_core.tools.root_step_check --root .
rtk python -m smart_grid_core.tools.parity_report --root .
```

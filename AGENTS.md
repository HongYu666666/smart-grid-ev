...
组员执行工作时只以已批准的 OpenSpec change 为范围和验收依据。发生冲突时，按以下顺序处理：

1. 当前 `openspec/changes/<change-id>/`：本次变更的范围、需求、设计、任务和验收标准。
2. 对应 GitHub Issue：指向 change/task，记录本次直接负责人、依赖和协作状态，不新增 spec 之外的要求。
3. `docs/仿真系统开发进度安排.md`：由 lead 用于规划和验收正式成果，不要求组员据此自行拆任务。
...
具体工作以当前 OpenSpec change 为准。组员身份和贡献由 Git 提交、Issue 和 PR 自然记录，不另建分工台账。每个 Issue 仍应有一个当次直接负责人，另一名组员或 lead 负责 Review。
